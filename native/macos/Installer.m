...
- (void)finished:(NSDictionary *)outcome {
    ...
    if (self.installed) {
        ...
        self.details.string = [self.plan[@"existing"] boolValue] ? IL(@"Jaso NFC is running with your saved folders, cleanup mode, login preference, and recovery history.\n\nOpen Jaso NFC. Choose Status in the sidebar for an overview, Activity for current work, or Folders to review your folders. For an access error, choose Settings → Advanced settings → Open Full Disk Access… and add Jaso NFC from Applications.", @"저장된 폴더, 정리 모드, 로그인 실행 선택과 복구 기록을 이어서 Jaso NFC를 실행했습니다.\n\nJaso NFC를 열고 창 왼쪽에서 ‘상태’로 전체 현황을, ‘활동’으로 현재 작업을 확인하세요. 정리할 폴더는 ‘폴더’에서 확인하세요. 접근 권한 오류가 표시되면 ‘설정 → 상세 설정 → 전체 디스크 접근 권한 열기…’를 누르고 응용 프로그램 폴더의 Jaso NFC를 추가하세요.") : IL(@"Open Jaso NFC to start cleaning up Downloads.\n\n1. Choose Folders in the sidebar and select the folders to clean up.\n2. Choose Preview filenames to compare current and proposed names.\n3. Choose Start automatic cleanup.\n\nCheck the overview in Status and current work in Activity. Jaso NFC will keep checking new files in your selected folders.", @"Jaso NFC를 열고 다운로드 폴더부터 정리해 보세요.\n\n1. 창 왼쪽의 ‘폴더’에서 원하는 폴더를 선택하세요.\n2. ‘파일명 미리보기’에서 현재 이름과 정리 후 이름을 확인하세요.\n3. ‘자동 정리 시작’을 누르세요.\n\n‘상태’에서 전체 현황을, ‘활동’에서 현재 작업을 확인하세요. 선택한 폴더에 이후 들어오는 파일도 계속 확인합니다.");
